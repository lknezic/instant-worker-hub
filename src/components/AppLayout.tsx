...
const AppLayout = () => {
  ...
  useEffect(() => {
    insights.timeSaved()
      .then((d) => {
        setTimeSaved(d.human_readable);
        setTimeSavedBreakdown(d.breakdown_display || []);
      })
  ...
  const sidebarContent = (
    <>
      ...
      {/* Time Saved */}
      {timeSaved && (
        <div className="group relative px-3 py-2 mx-3 mb-2 rounded-lg bg-primary/5 border border-primary/10">
          ...
          {/* Tooltip on hover */}
          {timeSavedBreakdown.length > 0 && (
            <div className="absolute bottom-full left-0 mb-2 hidden group-hover:block w-64 p-3 rounded-lg bg-card border border-border shadow-lg z-50">
              <p className="text-[10px] font-semibold text-muted-foreground mb-2 uppercase tracking-wider">How we calculated this</p>
              {timeSavedBreakdown.map((line, i) => (
